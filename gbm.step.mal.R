gbm.step.mal <- function (data, gbm.x, gbm.y, offset = NULL, fold.vector = NULL, 
    ...
    tolerance = 0.001, keep.data = FALSE, plot.main = TRUE, plot.folds = FALSE, 
    ...
    keep.fold.vector = FALSE, keep.fold.fit = FALSE, ...) 
{
    ...
    if (!silent) {
        ...
        cat("for", sp.name, "with dataframe", dataframe.name, 
            "and using a family of", family, "\n")
    ...
    for (i in 1:n.folds) {
        ...
        pred.values[pred.mask] <- predict.gbm(model.list[[i]], 
            x.data[pred.mask, ], n.trees = n.trees)
        if (!is.null(offset)) 
    ...
    while (delta.deviance > tolerance.test & n.fitted < max.trees) {
        ...
        for (i in 1:n.folds) {
            ...
            model.list[[i]] <- gbm.more(model.list[[i]], weights = weight.subset, 
                step.size)
            fitted.values <- model.list[[i]]$fit
            if (!is.null(offset)) 
                fitted.values <- fitted.values + offset[model.mask]
            ...
            if (family == "poisson") 
                fitted.values <- exp(fitted.values)
            pred.values[pred.mask] <- predict.gbm(model.list[[i]], 
                x.data[pred.mask, ], n.trees = n.fitted)
            if (!is.null(offset)) 
                pred.values[pred.mask] <- pred.values[pred.mask] + 
                  offset[pred.mask]
            if (family == "bernoulli") 
    ...
    browser() ### Debug Line  ### The loop above is where memory starts getting used up
    ...
    for (i in 1:n.folds) {
        pred.mask <- selector == i
        model.mask <- selector != i
        fits <- predict.gbm(model.list[[i]], x.data[model.mask, 
            ], n.trees = target.trees)
        if (!is.null(offset)) 
        ...
        fits <- predict.gbm(model.list[[i]], x.data[pred.mask, 
            ], n.trees = target.trees)
        if (!is.null(offset)) 
    ...
    if (verbose) {
        ...
        cat("cv correlation = ", round(cv.cor, 3), "; se =", 
            round(cv.cor.se, 3), "\n", "\n")
        if (family == "bernoulli") {
            cat("training data ROC score =", round(self.roc, 
                3), "\n")
            cat("cv ROC score =", round(cv.roc, 3), "; se =", 
                round(cv.roc.se, 3), "\n", "\n")
        }
    ...
    gbm.detail <- list(dataframe = dataframe.name, gbm.x = gbm.x, 
        predictor.names = names(x.data), gbm.y = gbm.y, response.name = sp.name, 
        offset = offset.name, family = family, tree.complexity = tree.complexity, 
        learning.rate = learning.rate, bag.fraction = bag.fraction, 
        cv.folds = n.folds, prev.stratification = prev.stratify, 
