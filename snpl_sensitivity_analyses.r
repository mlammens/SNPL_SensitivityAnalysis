## **************************************************************##
## File: snpl_sensitivity_analyses.r
...
snpl.run.brt <- function( snpl.brt.run, gbm.predictors, gbm.response, save.brt = FALSE,
  ...
  for ( Rep in 1:length(RepNum) ) {
    ...
    if ( save.brt ) {
      brt.file.name <- paste( brt.file.name, Rep, 'RData', sep="." )
...
snpl.run.mult.brt <- function( snpl.csv, brt.model.stor='/Users/Matthew/Projects/FL-Shore-Birds/BRT_Models/',
  ...
  ##
  ##   brt.file.base.name: Base name to be given to the stored brt model,
  ...
  # Run brt analysis on Part.Num. = 10k.SA2 scenario
...
#   ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#   # Call brt analysis function
#   brt.file.base.name <- paste( '~/Documents/StonyBrook-BEE/FL-shorebird/Sensitivity-Analysis-Data-Local/BRT_Models/snpl.10k.',
#                                df.num, sep="")
...
load( 'Results/snpl.10k.brt.results.RData' )
...
# Relative end-point metrics
brt.summary.relEnds.ref <- brt.summary.ord( snpl.10k.brt.results$rel.ends.lhs$brt.summary[[1]] )
...
#  ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#  # Call brt analysis function
#  brt.file.base.name <- paste( brt.model.stor,'snpl.100.',
#                               df.num, sep="")
...
## If the above has already been run, then just load in the
## results, which should have been saved to disk.
load( 'Results/snpl.100.brt.results.RData' )
...
#   ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#   # Call brt analysis function
#   brt.file.base.name <- paste( brt.model.stor,'snpl.500.',
#                                df.num, sep="")
...
## If the above has already been run, then just load in the
## results, which should have been saved to disk.
load( 'Results/snpl.500.brt.results.RData' )
...
#   ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#   # Call brt analysis function
#   brt.file.base.name <- paste( brt.model.stor,'snpl.1000.',
#                                df.num, sep="")
...
## If the above has already been run, then just load in the
## results, which should have been saved to disk.
load( 'Results/snpl.1000.brt.results.RData' )
...
#   ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#   # Call brt analysis function
#   brt.file.base.name <- paste( brt.model.stor,'snpl.250.',
#                                df.num, sep="")
...
# save( snpl.250.brt.results, file="Results/snpl.250.brt.results.RData" )

## If the above has already been run, then just load in the
## results, which should have been saved to disk.
load( 'Results/snpl.250.brt.results.RData' )
  
## **************************************************************##
## Section 8: BRT Analysis for Part.Num. = 10k - Modified sensitivity analysis

# # Run brt analysis on Part.Num. = 10k.SA2 scenario
# snpl.10k.SA2.rep <- read.csv('Results/snpl.10000.rep.SA2.csv')
# snpl.10k.SA2.part <- snpl.df.partition( snpl.10k.SA2.rep )
# # Clean-up
# rm( snpl.10k.SA2.rep )
# # Point to BRT_Model storage Location
# brt.model.stor <- '/Users/Matthew/Projects/FL-Shore-Birds/BRT_Models/'
# ###brt.model.stor <- '/Users/mlammens/Documents/StonyBrook-BEE/FL-shorebird/Sensitivity-Analysis-Data-Local/BRT_Models/'
# # Loop through the snpl.10k.SA2.part list, and execute the brt analysis
# snpl.10k.SA2.brt.results <- vector(mode="list",length=0)
# for ( df.num in 1:length(snpl.10k.SA2.part) ){
#   snpl <- snpl.10k.SA2.part[[ df.num ]]
#   # Determine which gbm.response variable to use
#   ifelse ( length(snpl) == 68, gbm.response <- abs.resp, gbm.response <- rel.resp )
#   # Call brt analysis function
#   brt.file.base.name <- paste( brt.model.stor,'snpl.10k.SA2.',
#                                df.num, sep="")
#   snpl.10k.SA2.brt.results[[ df.num ]] <- snpl.run.brt( snpl, gbm.predictors, gbm.response, 
#                                                     save.brt=TRUE, brt.file.basename=brt.file.base.name,
#                                                     learning.rate=0.01)
# }
# # Assign names to snpl.10k.SA2.brt.results
# names(snpl.10k.SA2.brt.results) <- names(snpl.10k.SA2.part)
# save( snpl.10k.SA2.brt.results, file="Results/snpl.10k.SA2.brt.results.RData" )

## If the above has already been run, then just load in the
## results, which should have been saved to disk.
load( 'Results/snpl.10k.SA2.brt.results.RData' )
  
## **************************************************************##
## Section 9: Relative Influence (Importance) data.frame construction
...
# Make a list of the different scenarios
snpl.brt.results <- list( snpl.100.brt.results, snpl.250.brt.results, snpl.500.brt.results, snpl.1000.brt.results )
...
## **************************************************************##
## Section 10: Correlation matrix construction
...
names( brt.corr.reference ) <- c( "cor.val", "samp.slr", "part.num",
                                  "samp","slr" )
brt.corr.reference$part.num <- factor( brt.corr.reference$part.num, c('snpl.100','snpl.250','snpl.500','snpl.1000') )
...
## I wanted to look at the distributions of various end-point metrics
## to see if any of them look like they have more variance than others.
## To do this, I first load in the data sets as per the first lines in 
## sections 3 to 6 above.  Then, use ggplot to have a look at the
## results.  Below is code used just for the Part.Num. = 10k scenario
snpl.10k.rep$kch.type <- factor( snpl.10k.rep$kch.type, c('LO.KCH','ME.KCH','HI.KCH'))
ggplot() + geom_histogram(aes(x=exp.min.n,colour=kch.type),data=snpl.10k.rep) + facet_grid(SLR~RandType)
ggplot() + geom_histogram(aes(x=prob.250,colour=kch.type),data=snpl.10k.rep) + facet_grid(SLR~RandType)
ggplot() + geom_histogram(aes(x=prob.50,colour=kch.type),data=snpl.10k.rep) + facet_grid(SLR~RandType)
# I was also curious to calculate the means of some of these values
# based on categories
ema.mean <- tapply( snpl.10k.rep$exp.min.n, INDEX=list(snpl.10k.rep$SLR,snpl.10k.rep$RandType,snpl.10k.rep$kch.type), mean )
prob.50.mean <- tapply( snpl.10k.rep$prob.50, INDEX=list(snpl.10k.rep$SLR,snpl.10k.rep$RandType,snpl.10k.rep$kch.type), mean )


## Below are functions found on stackoverload to help examine
## memory usage
